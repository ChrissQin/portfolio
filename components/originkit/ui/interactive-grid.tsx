...
interface ImageItem {
  src: string;
  /** Multiplier against the grid logoScale for this cell only (default 1). */
  scale?: number;
}
...
interface InteractiveGridProps {
  ...
  cardFill: string;
  cardBorder: string;
  /** Inner padding inside each logo card */
  cardPadding?: string;
  shadow: boolean;
  cardShadow: string;
  glow: boolean;
...
const scaleOf = (image: ImageItem | string): number =>
  typeof image === "string" ? 1 : (image?.scale ?? 1);
    ...
    >
      ...
      >
        {Array.from({ length: count }).map((_, i) => {
          ...
          const sourceImages = (images ?? []) as (ImageItem | string)[];
          const item =
            sourceImages.length > 0
              ? sourceImages[i % sourceImages.length]!
              : "";
          const itemLogoPct = Math.max(
            10,
            Math.min(200, logoPct * scaleOf(item)),
          );
          return (
            <div
              key={i}
